...
class Deepfm(nn.Module):
    ...
    def __init__(self, feat_sizes, sparse_feature_columns, dense_feature_columns, dnn_hidden_units=[400, 400, 400],
        ...
        self.embedding_dict2 = self.create_embedding_matrix(self.sparse_feature_columns, feat_sizes,
                                                            self.embedding_size,
                                                            sparse=False, device=self.device)
        # dnn
    ...
    def forward(self, X):
        ...
        dnn_total_input = dnn_sparse_input
        ...
        '''
            output
        '''
        y_pred = torch.sigmoid(logit + self.bias) * 5
        return y_pred

    def fit(self, train_input, y_label, val_input, y_val, batch_size=5000, epochs=150, verbose=5):
        ...
        for epoch in range(epochs):
            ...
            # if (epoch % verbose == 0):
            #     print('epoch %d train loss is %.4f ' %
            #           (epoch, total_loss_epoch / steps_per_epoch))
            #     self.val_auc_logloss(val_input, y_val, batch_size=5000)
            #     print(" ")
    ...
    def val_auc_logloss(self, val_input, y_val, batch_size=5000, use_double=False):

        pred_ans = self.predict(val_input, batch_size).squeeze()
        print("test LogLoss is %.4f" % (F.mse_loss(torch.tensor(y_val), torch.tensor(pred_ans), reduction='mean').item()))
